...
namespace Hooshabi.Client.Client.Pages.Identity.Roles;
...
public partial class RolePermissions
{
    ...
    static RolePermissions() => TypeAdapterConfig<HooshabiPermission, PermissionViewModel>.NewConfig().MapToConstructor(true);

    protected override async Task OnInitializedAsync()
    {
        var state = await AuthState;
        _canEditRoleClaims = await AuthService.HasPermissionAsync(state.User, HooshabiAction.Update, HooshabiResource.RoleClaims);
        _canSearchRoleClaims = await AuthService.HasPermissionAsync(state.User, HooshabiAction.View, HooshabiResource.RoleClaims);
            ...
            is RoleDto role && role.Permissions is not null)
        {
            ...
            var permissions = state.User.GetTenant() == MultitenancyConstants.Root.Id
                ? HooshabiPermissions.All
                : HooshabiPermissions.Admin;
...
public record PermissionViewModel : HooshabiPermission
{
