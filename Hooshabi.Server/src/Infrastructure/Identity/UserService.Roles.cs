...
internal partial class UserService
{
    ...
    public async Task<string> AssignRolesAsync(string userId, UserRolesRequest request, CancellationToken cancellationToken)
    {
        ...
        if (await _userManager.IsInRoleAsync(user, HooshabiRoles.Admin)
            && request.UserRoles.Any(a => !a.Enabled && a.RoleName == HooshabiRoles.Admin))
        {
            // Get count of users in Admin Role
            int adminCount = (await _userManager.GetUsersInRoleAsync(HooshabiRoles.Admin)).Count;
