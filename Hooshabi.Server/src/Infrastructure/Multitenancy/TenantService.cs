...
{
    private readonly IMultiTenantStore<HooshabiTenantInfo> _tenantStore;
    ...
    public TenantService(
        IMultiTenantStore<HooshabiTenantInfo> tenantStore,
    ...
    public async Task<string> CreateAsync(CreateTenantRequest request, CancellationToken cancellationToken)
    {
        ...
        var tenant = new HooshabiTenantInfo(request.Id, request.Name, request.ConnectionString, request.AdminEmail, request.Issuer);
    ...
    private async Task<HooshabiTenantInfo> GetTenantInfoAsync(string id) =>
        await _tenantStore.TryGetAsync(id)
            ?? throw new NotFoundException(string.Format(_localizer["entity.notfound"], typeof(HooshabiTenantInfo).Name, id));
}